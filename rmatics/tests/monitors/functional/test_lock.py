...
class TestRedisLocker(TestCase):
    ...
    def test_unlock_after(self):
        ...
        self.another_locker.lock(RESOURCE_KEY, LOCK_TIME_MSEC)

        self.lock.lock(RESOURCE_KEY, LOCK_TIME_MSEC)
        time_stop = time.time()
        ...
        # Actually it is side effect so we should realise RedisLocker
        self.lock.unlock(RESOURCE_KEY)
